const Parkingarea = require("../models/parkingModels.js");
    ...
    Parkingarea.getAll((err, data) => {
        if (err)
            ...
            request(options, function (err, body) {
                ...
                Parkingarea.findByPosition(body.body.sources[indiceMin].location[0], body.body.sources[indiceMin].location[1], (err, data) => {
                    if (data == null) {
                        res.status(400).send({
                            message: `park not found .`
                        });
                    } else {
                        parcheggio = new Parkingarea(data);
                        parcheggio.status = 1;
                        parcheggio.emailDriver = req.decoded.email
                        console.log(data.idParkingarea + " parcheggio")
                        Parkingarea.updateById(data.idParkingarea, parcheggio,
                            (err, datachanged) => {
                                if (err) {
                                    ...
                                    var park = [body.body.sources[indiceMin].location, data.idParkingarea]
                                    console.log(park)
                                    res.send(park[0]);
                                }
...
exports.parked = (req, res) => {
    Parkingarea.findbyemail(req.decoded.email, (err, data) => {
            ...
            Parkingarea.updateById(data.idParkingarea, parcheggio,
                (err, result) => {
                    if (err) {
                        if (err.kind === "not_found") {
                            res.status(404).send({
                                message: `Not found Parking slot with id ${data.idParkingarea}.`
                            });
                        } else {
                            res.status(500).send({
                                message: "Error updating Customer with id " + data.idParkingarea
                            });
                        }

                    } else { //N.B. non aspetta la risposta dell'email
                        send.notification([data.idParkingarea, data.coord])
                        res.send().status(200);
                    }
                });
        } else {
...
exports.malfunction = (req, res) => {
    Parkingarea.findbyemail(req.decoded.email, (err, data) => {
        ...
        if (err) {
            ...
            send.notifyMalfunction([data.idParkingarea, data.coord])
            res.send().status(200);
        }
    })
}
...
exports.findOne = (req, res) => {
    ...
    */
    Parkingarea.findById(req.params.parkingId, (err, data) => {
...
exports.update = (req, res) => {
    ...
    */
    ...
    Parkingarea.updateById(req.params.parkingId, new Parkingarea(req.body), (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                res.status(404).send({
...
exports.delete = (req, res) => {
    ...
    */
    Parkingarea.remove(req.params.parkingId, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                res.status(404).send({
                    message: `Not found Parking slot with id ${req.params.parkingId}.`
